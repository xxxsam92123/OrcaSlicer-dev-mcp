...
Surfaces split_internal_solid_grid_surface(const Surface &surface, const InternalSolidGridSettings &settings)
{
    ...
    for (int y = 0; y < cells_y; ++y)
        for (int x = 0; x < cells_x; ++x) {
            ...
            // The cells form an exact partition of bbox, with only zero-area shared
            // boundaries. Intersecting the complete ExPolygon preserves holes and
            // makes every emitted fragment disjoint without contour-only clipping.
    ...
    // A surface that occupies only one grid cell needs no synthetic partition.
    return result.size() <= 1 ? full_surface(surface) : result;
}
